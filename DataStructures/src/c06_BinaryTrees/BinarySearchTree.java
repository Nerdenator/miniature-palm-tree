...
public class BinarySearchTree extends AbstractBinaryTree<Integer> {
	 ...
	 */
	protected Node<Integer> findParentIterative(Integer data) {
	...
	public Node<Integer> delete(Integer data) {
		...
		// replace cur with its in-order successor (leftmost child leaf of the right subtree)

		// find in-order successor
		Node<Integer> suc = cur.right;
		// if suc has no left children, then it's the smallest node in the right subtree
		if (suc.left != null) {
			...
			//move the right child of the deleted node to the right child of the successor
			suc.right = cur.right;
		}
