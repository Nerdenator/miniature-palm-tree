...
public class BinaryTree<MyType> extends AbstractBinaryTree<MyType> {
	...
	public int insertUsingBinaryCode(MyType data, int codeVal) {
		...
		// the binary representation of codeValue, starting at 4, except the leading 1
	 ...
	 * @return
	 */
	protected static String convertToBinary(int numNodes) {
		if (numNodes == 0)
			return "0";
	...
	public Node<MyType> delete(MyType data) {
		...
		if (parent.left.data == data) {
			...
			if (node.left == null && node.right == null)
				parent.left = null;
			// if it only has a left child, set that child as a child of the parent
			else if (node.left == null)
				parent.left = node.right;
			// if it only has a right child, set that child as a child of the parent
			else if (node.right == null)
				parent.left = node.left;
			// if it has 2 children, make the left child of the node the left child 
			// of the parent, and the right child of the node the rightmost child
			else {
				parent.left = node.left;
			...
			// if it has 2 children, make the right child of the node the right child 
