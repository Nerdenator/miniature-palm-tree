...
public class HashTableUtil {
	 ...
	 * @return
	 */
	protected static int getPreviousPrime(int x) {
		if (isPrime(x))
			x--;
	 ...
	 * @return
	 */
	protected static int getNextPrime(int x) {
		if (x == 1)
			return 2;
	...
	protected static Boolean isPrime(int x) {
		...
		// check if x is divisible by i, and if so it's not prime (check until sqrt(x))
		for (int i = 2; i * i <= x; i++)
			if (x % i == 0)
				return false;
